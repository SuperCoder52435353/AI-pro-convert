...
const Brain = {
    ...
    async processFile(file) {
        ...
        if (file.size > 50 * 1024 * 1024) {
            Utils.notify('Fayl juda katta! Maksimal 50MB', 'error');
            return;
        }
        ...
        try {
            ...
            } else if (['png', 'jpg', 'jpeg', 'gif'].includes(this.fileType)) {
            ...
            AI.addMessage('ai', `✅ <strong>Fayl muvaffaqiyatli yuklandi!</strong><br><br>📄 <strong>Nomi:</strong> ${file.name}<br>📊 <strong>Hajmi:</strong> ${fileSize}<br>📋 <strong>Format:</strong> ${this.fileType.toUpperCase()}<br><br>🎯 Endi qaysi formatga o'tkazmoqchisiz? Pastdagi tugmalardan birini tanlang! 👇`);
    ...
    async convertTo(targetFormat) {
        ...
        try {
            ...
            AI.addMessage('ai', `🎉 <strong>Muvaffaqiyatli!</strong><br><br>✅ <strong>${filename}</strong> yuklab olindi!<br><br>📊 Format: ${targetFormat.toUpperCase()}<br>💾 Hajm: ${Utils.formatFileSize(blob.size)}<br><br>🔄 Yana fayl convert qilmoqchimisiz? Yuqoridan yangi fayl yuklang!`);
